...
namespace BillSoft.Api.Dashboard;
...
public static class OwnerDashboardEndpoints
{
    public static IEndpointRouteBuilder MapOwnerDashboardEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/owner", GetOwnerDashboardAsync)
            .RequirePermission(SystemPermissions.ReportView);

        return app;
    }

    private static async Task<IResult> GetOwnerDashboardAsync(
        string? date,
        Guid? branchId,
        ...
        CancellationToken cancellationToken)
    {
        return await ExecuteAsync(
            async () => await ownerDashboardService.GetOwnerDashboardAsync(
