...
export interface BillReceiptErrorMessages {
  sessionExpired: string;
  unauthorized: string;
}

const defaultMessages: BillReceiptErrorMessages = {
  sessionExpired: 'Your session expired. Please sign in again.',
  unauthorized: 'You are not authorized to use bill receipts.',
};

export const getSafeBillReceiptErrorMessage = (
  error: unknown,
  fallback: string,
  messages: BillReceiptErrorMessages = defaultMessages
): string => {
  if (isApiError(error)) {
    if (error.status === 401) {
