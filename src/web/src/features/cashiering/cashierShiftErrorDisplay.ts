...
export interface CashierShiftErrorMessages {
  sessionExpired: string;
  unauthorized: string;
}

const defaultMessages: CashierShiftErrorMessages = {
  sessionExpired: 'Your session expired. Please sign in again.',
  unauthorized: 'You are not authorized to use cashier shifts.',
};

export const getSafeCashierShiftErrorMessage = (
  error: unknown,
  fallback: string,
  messages: CashierShiftErrorMessages = defaultMessages
): string => {
  if (isApiError(error)) {
    if (error.status === 401) {
