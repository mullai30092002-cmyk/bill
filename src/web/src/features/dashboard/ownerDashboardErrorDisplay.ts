...
export interface OwnerDashboardErrorMessages {
...
const defaultErrorMessages: OwnerDashboardErrorMessages = {
  sessionExpired: 'Your session expired. Please sign in again.',
  unauthorized: 'You are not authorized to view the owner dashboard.',
};

export const getSafeOwnerDashboardErrorMessage = (
  error: unknown,
  fallback: string,
  messages: OwnerDashboardErrorMessages = defaultErrorMessages
): string => {
  if (isApiError(error)) {
    if (error.status === 401) {
