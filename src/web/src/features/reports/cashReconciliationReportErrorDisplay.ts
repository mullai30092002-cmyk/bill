...
export interface CashReconciliationReportErrorMessages {
...
const defaultErrorMessages: CashReconciliationReportErrorMessages = {
  sessionExpired: 'Your session expired. Please sign in again.',
  unauthorized: 'You are not authorized to view reports.',
};

export const getSafeCashReconciliationReportErrorMessage = (
  error: unknown,
  fallback: string,
  messages: CashReconciliationReportErrorMessages = defaultErrorMessages
): string => {
  if (isApiError(error)) {
    if (error.status === 401) {
