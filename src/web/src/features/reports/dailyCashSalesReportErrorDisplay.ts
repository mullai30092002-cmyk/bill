...
export interface DailyCashSalesReportErrorMessages {
...
const defaultErrorMessages: DailyCashSalesReportErrorMessages = {
  sessionExpired: 'Your session expired. Please sign in again.',
  unauthorized: 'You are not authorized to view reports.',
};

export const getSafeDailyCashSalesReportErrorMessage = (
  error: unknown,
  fallback: string,
  messages: DailyCashSalesReportErrorMessages = defaultErrorMessages
): string => {
  if (isApiError(error)) {
    if (error.status === 401) {
