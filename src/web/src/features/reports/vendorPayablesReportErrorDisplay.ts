...
export interface VendorPayablesReportErrorMessages {
...
const defaultErrorMessages: VendorPayablesReportErrorMessages = {
  sessionExpired: 'Your session expired. Please sign in again.',
  unauthorized: 'You are not authorized to view reports.',
};

export const getSafeVendorPayablesReportErrorMessage = (
  error: unknown,
  fallback: string,
  messages: VendorPayablesReportErrorMessages = defaultErrorMessages
): string => {
  if (isApiError(error)) {
    if (error.status === 401) {
