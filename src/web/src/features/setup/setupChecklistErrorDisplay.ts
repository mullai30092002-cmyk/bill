...
export interface SetupChecklistErrorMessages {
...
const defaultMessages: SetupChecklistErrorMessages = {
  sessionExpired: 'Your session expired. Please sign in again.',
  unauthorized: 'You are not authorized to view the setup checklist.',
};

export const getSafeSetupChecklistErrorMessage = (
  error: unknown,
  fallback: string,
  messages: SetupChecklistErrorMessages = defaultMessages
): string => {
  if (isApiError(error)) {
    if (error.status === 401) {
